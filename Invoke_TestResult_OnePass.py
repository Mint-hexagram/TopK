...
for _ in range(1000):
    ...
    result_Basic = subprocess.run(['./TestResult_OnePass.exe',str(n),str(w),str(s),str(k)], capture_output=True, text=True) 
    ...
    result = compare_files('Basic_Moving.txt','OnePass.txt')
    correct_ans+=1
